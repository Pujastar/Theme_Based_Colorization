...
# default arguments
CONTENT_WEIGHT = 5e0
CONTENT_WEIGHT_BLEND = 1 #specifies the coefficient of content transfer layers. 
#Default value - 1.0, style transfer tries to preserve finer grain content details. The value should be in range [0.0; 1.0].

STYLE_WEIGHT = 5e2
TV_WEIGHT = 1e2 #total variation regularization weight 

STYLE_LAYER_WEIGHT_EXP = 1 #how "abstract" the style transfer should be. 
#Lower values mean that style transfer of a finer features will be favored over style transfer of a 
#more coarse features, and vice versa. Default value is 1.0 - all layers treated equally.

LEARNING_RATE = 1e1
BETA1 = 0.9 #Adam: beta1 parameter
BETA2 = 0.999 #Adam: beta2 parameter
EPSILON = 1e-08 #Adam: epsilon parameter 
STYLE_SCALE = 1.0 #one or more style scales
ITERATIONS = 1000
VGG_PATH = "/content/drive/My Drive/styletransfer/imagenet-vgg-verydeep-19.mat"
POOLING = 'max'
network=VGG_PATH
preserve_colors=False
progress_write=True #write iteration progess data to OUTPUT's dir
progress_plot =True #plot iteration progess data to OUTPUT's dir
def build_parser():
...
def main():
    ...
    TEST_PATH = "/content/drive/My Drive/deep_learning_project/test_images/"
    OUTPUT_PATH = "/content/drive/My Drive/deep_learning_project/output_images/"
    loss_arrs = None
    for iteration, image, loss_vals in stylize(
        ...
        if (image is not None) and (options.checkpoint_output is not None):
            imsave(TEST_PATH+fmt_imsave(options.checkpoint_output, iteration), image)
            print(iteration,loss_vals)
        if (loss_vals is not None) and (progress_plot or progress_write):
    ...
    imsave(OUTPUT_PATH+options.output, image)

    if progress_write:
        fn = "/content/drive/My Drive/deep_learning_project/graphs/"+options.output+"progress.txt".format(os.path.dirname(options.output))
    ...
    if progress_plot:
        ...
        ax.set_ylabel("loss")
        fig.savefig("/content/drive/My Drive/deep_learning_project/graphs/"+options.output+"progress.png".format(os.path.dirname(options.output)))
